...
# ==========================================
# 2. DEFINICIÓN DE MAPEOS Y TRADUCCIONES
# ==========================================
tarte_mapping = {
...
tarte_mapping_cortical_4 = {
    ...
    'Bankssts_CV_MEAN': 'Banks of the Superior Temporal Sul Sulcus Cortical Volume Mean',
...
def traducir_freesurfer(nombre_columna):
    match = re.search(r'ST(\d+)([A-Z]{2})_', nombre_columna)
    
    if not match:
        return nombre_columna 
        
    region_id = match.group(1)
    metrica_code = match.group(2)
    ...
    texto_metrica = metricas.get(metrica_code, 'Measurement')
    return f"Brain MRI Region {region_id} {texto_metrica}"

def aplicar_mapeo_tarte(df, mapeo_maestro):
    nuevas_columnas = []
    for col in df.columns:
        if col in mapeo_maestro:
            nuevas_columnas.append(mapeo_maestro[col])
        else:
            nuevas_columnas.append(traducir_freesurfer(col))
            
    df_mapped = df.copy()
    df_mapped.columns = nuevas_columnas
    
    # Desduplicar nombres de columnas para evitar que skrub/tarte-ai fallen al recibir DataFrames en vez de Series
    cols_unicas = []
    counts = {}
    for col in df_mapped.columns:
        if col in counts:
            counts[col] += 1
            cols_unicas.append(f"{col} ({counts[col]})")
        else:
            counts[col] = 0
            cols_unicas.append(col)
            
    df_mapped.columns = cols_unicas
    return df_mapped

# ==========================================
# 3. PREPROCESAMIENTO ESTRICTO
# ==========================================
...
# ==========================================
print("Cargando y preprocesando datos reales de TADPOLE...")
...
# Aplicar el mapeo a las características
X_train = aplicar_mapeo_tarte(X_train, mapeo_maestro)
X_val   = aplicar_mapeo_tarte(X_val, mapeo_maestro)
X_test  = aplicar_mapeo_tarte(X_test, mapeo_maestro)

print(f"\nDimensiones -> TRAIN: {X_train.shape}, VAL: {X_val.shape}, TEST: {X_test.shape}")

# Configurar PredefinedSplit para entrenamiento/validación
...
print("\n[+] Inicializando TARTE y extrayendo embeddings del modelo fundacional...")
start_emb = time.time()
tarte_model = TARTEFeaturizer()

print(" -> Extrayendo embeddings semánticos para TRAIN + VAL...")
X_train_val_emb = tarte_model.fit_transform(X_train_val)
print(f" -> Completado en {time.time() - start_emb:.2f} segundos.")

start_test_emb = time.time()
print(" -> Extrayendo embeddings semánticos para TEST...")
X_test_emb = tarte_model.transform(X_test)
print(f" -> Completado en {time.time() - start_test_emb:.2f} segundos.")
print(f"[+] Extracción de embeddings completada en {(time.time() - start_emb) / 60:.2f} minutos.")

X_train_val_orig_np = np.array(X_train_val)
X_test_orig_np = np.array(X_test)

# ==========================================
# 6. FUNCIÓN DE EXPERIMENTOS SVM EXHAUSTIVO
# ==========================================
def entrenar_y_evaluar_svm(nombre_experimento, X_tr, y_tr, X_te, y_te, cv_splitter):
    print(f"\n{'='*65}")
    ...
    # GridSearchCV usando PredefinedSplit
    grid_search = GridSearchCV(
        estimator=pipeline,
        param_grid=param_grid,
        scoring='f1_macro',
        cv=cv_splitter,
        n_jobs=-1,          # Paralelización sobre todos los núcleos
        verbose=2,
        refit=True
    )
    ...
    print(f"\n[+] Búsqueda finalizada en {tiempo_total:.2f} min.")
    print(f"[+] Mejores Hiperparámetros: {grid_search.best_params_}")
    
    y_pred = mejor_modelo.predict(X_te)
    y_prob = mejor_modelo.predict_proba(X_te)
    ...
    disp.plot(cmap="Blues" if "Boost" not in nombre_experimento else "Oranges", ax=ax_cm, values_format='d')
    plt.title(f"Matriz de Confusión - {nombre_experimento} (Real)", fontsize=12)
    plt.xlabel("Diagnóstico Predicho")
    plt.ylabel("Diagnóstico Real")
    plt.tight_layout()
    plt.savefig(f"Matriz_Confusion_{nombre_experimento.replace(' + ', '_').replace(' ', '_')}_Real.png", dpi=300)
    plt.close()

    # Curva ROC Multiclase (One-vs-Rest)
    y_test_bin = label_binarize(y_te, classes=range(len(le.classes_)))
    ...
    for i, color in zip(range(n_classes), colors):
        ...
        ax_roc.plot(fpr, tpr, color=color, lw=lw,
                    label=f'Curva ROC para {le.classes_[i]} (AUC = {roc_auc_val:0.3f})')
    ax_roc.plot([0, 1], [0, 1], 'k--', lw=lw)
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.xlabel('Tasa de Falsos Positivos (1 - Especificidad)')
    plt.ylabel('Tasa de Verdaderos Positivos (Sensibilidad / Recall)')
    plt.title(f'Curva ROC Multiclase - {nombre_experimento}', fontsize=12)
    plt.legend(loc="lower right")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"Curva_ROC_{nombre_experimento.replace(' + ', '_').replace(' ', '_')}_Real.png", dpi=300)
    plt.close()
    
    return {
        'Método': nombre_experimento,
        'Accuracy': acc,
...
# --- Experimento 1: TARTE Features + SVM ---
res_exp1 = entrenar_y_evaluar_svm(
    nombre_experimento="TARTE Features + SVM",
    X_tr=X_train_val_emb, 
    y_tr=y_train_val, 
    X_te=X_test_emb, 
    y_te=y_test,
    cv_splitter=ps
)
resultados.append(res_exp1)

# --- Experimento 2: TARTE Boost + SVM ---
X_train_val_boost = np.hstack([X_train_val_orig_np, X_train_val_emb])
X_test_boost = np.hstack([X_test_orig_np, X_test_emb])

res_exp2 = entrenar_y_evaluar_svm(
    nombre_experimento="TARTE Boost + SVM",
    X_tr=X_train_val_boost, 
    y_tr=y_train_val, 
    X_te=X_test_boost, 
    y_te=y_test,
    cv_splitter=ps
)
resultados.append(res_exp2)
...
print(f"\n{'='*75}")
print("TABLA COMPARATIVA FINAL DE RESULTADOS REALES (TADPOLE)")
print(f"{'='*75}")
df_resultados = pd.DataFrame(resultados)
print(df_resultados.to_markdown(index=False, floatfmt=".4f"))

# Guardar los archivos resultantes
df_resultados.to_csv("Resultados_Comparacion_Real.csv", index=False)
print("\n¡Resultados guardados y gráficas exportadas correctamente en tu espacio de trabajo!")
